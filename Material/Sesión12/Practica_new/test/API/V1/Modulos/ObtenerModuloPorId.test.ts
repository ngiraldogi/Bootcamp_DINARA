...
test.group('Modulos', (group) => {
  ...
  test('Espera obtener un modulo por id', async (assert) => {
    const { text } = await supertest(BASE_URL).get('/2').set('Authorization', `Bearer ${await obtenerTokenAutorizacion()}`).expect(200)
