...
test.group('Encuestas', (group) => {
  ...
  test('Espera que liste todas las encuestas', async (assert) => {
    const { text } = await supertest(BASE_URL).get('/listartodas').set('Authorization', `Bearer ${await obtenerTokenAutorizacion()}`).expect(200)
